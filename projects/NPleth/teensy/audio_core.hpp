...
//	int16_t  data[AUDIO_BLOCK_SAMPLES] = {};
	...
	void zeroAudioBlock() {
		std::memset(data, 0, sizeof(int16_t) * AUDIO_BLOCK_SAMPLES);

	}

	static void copyBlock(const audio_block_struct* src, audio_block_struct* dst) {
		if (src && dst) {
			std::memcpy(&(dst->data), &(src->data), AUDIO_BLOCK_SAMPLES);
		}
	}
} audio_block_t;
...
class AudioSynthNoiseWhiteFloat {
public:
	AudioSynthNoiseWhiteFloat() { }
...
class AudioSynthNoiseGritFloat {
public:
	AudioSynthNoiseGritFloat() { }
